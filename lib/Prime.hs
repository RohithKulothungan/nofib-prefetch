...
singleTestX n (k, q) x =
  ...
  where
    (t:ts) = take (fromInteger k) (iterate square (powerMod x q n))
    witness [] = False
    witness (t:ts) =
        if t == n - 1 then
            True
...
-- range 0 <= x < n, and the remainder of the pseudo-random numbers.
...
uniform (n:ns) (r:rs) =
    if t == n then
        ...
        t : map ((`mod` 65536) . toInteger) rs
  where
    t = toInteger r `mod` (n + 1)